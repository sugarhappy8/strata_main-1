...
test("a Strata+ member builds, tracks, reloads, and safely refreshes a coaching week",{timeout:60_000},async()=>{
  ...
  try{
    ...
    assert.equal(profileBody.profile.trainingGoal,"strength");await page.locator("#coachingDashboard").waitFor({state:"visible"});assert.equal(await page.locator("#nutritionWorkspaceTitle").evaluate((node)=>node===globalThis.document.activeElement),true,"successful generation should focus the visible dashboard heading");
    ...
    await page.locator('.destination-link[data-feature-target="plan"]').click();await page.getByText("Training coverage",{exact:true}).click();await page.click("#programApply");await page.locator("#programApplyDialog[open]").waitFor();const beforeApply=await(await context.request.get("/api/plan")).json();assert.equal(Object.values(beforeApply.plan.days).flat().length,0,"reviewing must not save the suggestion");await page.click("#programApplyCancel");await page.click("#programApply");const applyResponse=page.waitForResponse(r=>new URL(r.url()).pathname==="/api/plan"&&r.request().method()==="PUT");await page.click("#programApplyConfirm");const applied=await applyResponse;assert.equal(applied.status(),200,await applied.text());const appliedWeek=await applied.json();assert.equal(Object.values(appliedWeek.plan.days).flat().length,profileResult.week.training.sessions.reduce((sum,session)=>sum+session.exercises.length,0));await page.locator("#programTrainLink").waitFor({state:"visible"});assert.equal(await page.locator("#programTrainLink").getAttribute("href"),"/workout.html");await page.locator('.destination-link[data-feature-target="nutrition"]').click();assert.match((await page.locator("#coachingTrainingCoverage").textContent())||"",/direct sets over/);assert.match((await page.locator("#coachingCalibrationAdjustment").textContent())||"",/equation baseline/);
    ...
    await page.locator('.destination-link[data-feature-target="progress"]').click();await page.locator("#progressWorkspace").waitFor({state:"visible"});assert.equal(await page.locator("#progressCoachingLogForm").count(),0);assert.equal(await page.locator("#progressWorkspace form").count(),0);await page.locator('#progressWorkspace [data-feature-target="nutrition"]').click();assert.equal(await page.locator("#coachingCaloriesEaten").inputValue(),"1800");assert.equal(await page.locator("#coachingMorningWeight").inputValue(),"82.1");
    let releaseFood;const foodGate=new Promise((resolveGate)=>{releaseFood=resolveGate;});await page.route("**/api/coaching/food-options/**",async(route)=>{await foodGate;await route.continue();});const refreshedFood=page.waitForResponse((response)=>new URL(response.url()).pathname.startsWith("/api/coaching/food-options/")&&response.request().method()==="GET");await page.click("#coachingFoodRefresh");await page.waitForFunction(()=>globalThis.document.querySelector("#coachingFoodOptions")?.getAttribute("aria-busy")==="true");assert.equal((await page.locator("#coachingFoodOptions").textContent())?.trim(),"","stale meal cards must clear while a refreshed day is being matched");releaseFood();assert.equal((await refreshedFood).status(),200);await page.locator("#coachingFoodOptions .coaching-meal-card").first().waitFor();assert.equal(await page.locator("#coachingFoodTitle").evaluate((node)=>node===globalThis.document.activeElement),true,"Progress refresh should focus its own visible result heading");await page.unroute("**/api/coaching/food-options/**");
    await page.fill("#coachingCaloriesEaten","1825");await page.fill("#coachingMorningWeight","82.0");await page.fill("#coachingProteinEaten","151");await page.fill("#coachingCarbsEaten","201");await page.fill("#coachingFatEaten","61");const progressSavePromise=page.waitForResponse((response)=>new URL(response.url()).pathname===`/api/coaching/logs/${date}`&&response.request().method()==="PUT");await page.click("#coachingSaveLog");const progressSave=await progressSavePromise;assert.equal(progressSave.status(),200,await progressSave.text());assert.equal(progressSave.request().postDataJSON().expectedRevision,1);assert.deepEqual(progressSave.request().postDataJSON().log,{calories:1825,proteinG:151,carbsG:201,fatG:61,morningWeightKg:82,complete:true});assert.equal((await progressSave.json()).log.revision,2);assert.match((await page.locator("#coachingProgressSummary").textContent())||"",/1,825 kcal/);await page.locator('.destination-link[data-feature-target="nutrition"]').click();await page.locator("#coachingDashboard").waitFor({state:"visible"});
...
test("an equipment-limited beginner sees honest training gaps while calories and diary remain usable",async()=>{
  ...
  try{
    ...
    const page=await context.newPage();await page.goto("/discover.html#nutritionWorkspace",{waitUntil:"domcontentloaded"});await page.locator("#coachingDashboard").waitFor({state:"visible"});
